...
import com.project.phoneshop.model.Brand_;
import com.project.phoneshop.model.Model;
import com.project.phoneshop.model.Model_;
...
public class ModelSpec implements Specification<Model> {
	...
	public Predicate toPredicate(Root<Model> model, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if (modelFilter.getBrandId() != null) {
			Predicate brandId = brand.get(Brand_.ID).in(modelFilter.getBrandId());
			list.add(brandId);
		}

		if (modelFilter.getBrandName() != null) {
			// brand.get("id").in(modelFilter.getBrandId());
